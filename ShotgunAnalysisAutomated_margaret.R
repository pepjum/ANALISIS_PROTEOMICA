...
if (error == 0)
{
	...
	}else
	{
			...
			dataPSMMat <- data.frame()
				for (j in 1:nrow(experimentData))
				{
					if (j == 1)
					{
						...
						{
							dataPSMMat <-  data.frame("file" = paste(experimentData[1,1]), parseOmssaStandardOutput_SEP19(paste(experimentData[1,1])), "database" = paste(experimentData[1,2]), "sample" = paste(experimentData[1,3]), "fraction" = paste(experimentData[1,4]), "datfile" = paste(experimentData[1,5]))
						}
						else
						{
						...
						{
							dataPSMMat <- rbind(dataPSMMat, data.frame("file" = paste(experimentData[j,1]), parseXTandemStandardOutput_v_NEW(paste(experimentData[j,1])), "database" = paste(experimentData[j,2]), "sample" = paste(experimentData[j,3]), "fraction" = paste(experimentData[j,4]), "datfile" = paste(experimentData[j,5])))
						}
						else if (search_engine == 4)
						{
							dataPSMMat <- rbind(dataPSMMat, data.frame("file" = paste(experimentData[j,1]), parseOmssaStandardOutput_SEP19(paste(experimentData[j,1])), "database" = paste(experimentData[j,2]), "sample" = paste(experimentData[j,3]), "fraction" = paste(experimentData[j,4]), "datfile" = paste(experimentData[j,5])))
						}
					}
				}
			...
			if  (search_engine == 1)
			{
				...
				dataPSMMat$Scan_tmp<-lapply(strsplit(paste(dataPSMMat$ScanString)," "),"[",1)
				dataPSMMat$Scan_tmp2<-lapply(strsplit(paste(dataPSMMat$Scan_tmp),"\\."),"[",2)
				dataPSMMat$Query_tmp<-lapply(strsplit(paste(dataPSMMat$Scan_tmp),"\\."),"[",1)
				dataPSMMat$ScanNumber<-dataPSMMat$Scan_tmp2
				dataPSMMat$Query<-paste(dataPSMMat$Query_tmp,dataPSMMat$Scan_tmp2,sep="_")
				dataPSMMat$Scan_tmp<-NULL
				dataPSMMat$Scan_tmp2<-NULL
				dataPSMMat$Query_tmp<-NULL
				dataPSMMat$Rank<-NULL
				dataPSMMat$PSM<-paste(dataPSMMat$Query, dataPSMMat$PeptideSeq, sep="_")
				dataPSMMat<-unique(dataPSMMat)
				dataPSMMat$PSM<-str_replace(paste(dataPSMMat$PSM), "%2d", "-")
			...
			} else if (search_engine == 2)
			{
				...
				dataPSMMat$Fraction<-lapply(strsplit(paste(dataPSMMat$datfile),"_corrected"),"[",1)
				dataPSMMat$MGFfile<-paste0(dataPSMMat$Fraction,".mgf")
				dataPSMMat$Query_tmp<-NULL
				dataPSMMat$mgfIndex<-dataPSMMat$ScanNumber
				dataPSMMat$ScanNumber<-NA
				dataPSMMat$dummyCode<-paste(dataPSMMat$Fraction,dataPSMMat$mgfIndex,sep="_")

				tmp_experiment<-dirname(paste(dataPSMMat$ScanString[1]))
				tmp_experiment2<-dirname(tmp_experiment)
				#tmp_experiment3<-dirname(tmp_experiment2)
				#tmp_experiment4<-paste(tmp_experiment3,"MGFfiles/", sep="/")
				cat("tmp_experiment2  ", tmp_experiment2,"\n")
				cat("loading trazability Rdata","\n")
				trazability_out<-get(load(paste0(tmp_experiment2,"/trazability_",dataset,".Rdata")))

				cat("merge Rdata","\n")


				Comet_merged<-merge(dataPSMMat,trazability_out, by="dummyCode", all.x=TRUE)
				#Comet_merged$ScanNumber<-gsub("[^0-9.]", "",  Comet_merged$scanstring) #para todos loe xperimentos menos los de la maquina 5600
				Comet_merged$ScanNumber<-Comet_merged$scanstring
				...
				dataPSMMat<-Comet_merged
				dataPSMMat<-unique(dataPSMMat)
				cat("example of results_merged  ", dataPSMMat$PSM[1], "\n")

			} else if (search_engine == 3)
			{
				#dataPSMMat<-dataPSMMat[complete.cases(dataPSMMat),]
				dataPSMMat$search_engine <- "tandem"
				search_engine_str = "tandem"

				dataPSMMat$Query_tmp<-lapply(strsplit(paste(dataPSMMat$ScanString),"\\."),"[",1)
				dataPSMMat$Query<-paste(dataPSMMat$Query_tmp,dataPSMMat$ScanNumber,sep="_")
				dataPSMMat$Query_tmp<-NULL
				dataPSMMat$PSM<-paste(dataPSMMat$Query, dataPSMMat$PeptideSeq, sep="_")
			...
			} else if (search_engine == 4)
			{
				...
				dataPSMMat$Query_tmp<-lapply(strsplit(paste(dataPSMMat$ScanString),"\\."),"[",1)   #bueno para la mayoria de experimentos (menos 5600)
				#dataPSMMat$Query_tmp<-lapply(strsplit(paste(dataPSMMat$datfile),"_corrected"),"[",1)

				dataPSMMat$ScanNumber<-lapply(strsplit(paste(dataPSMMat$ScanString), "\\."),"[",2)    #para la mayoria de experimentos (menos 5600)
				#dataPSMMat$ScanNumber<-lapply(strsplit(paste(dataPSMMat$ScanString), ";"),"[",3)
				#dataPSMMat$ScanNumber<-lapply(strsplit(paste(dataPSMMat$ScanString), " "),"[",1)

				#dataPSMMat$ScanNumber<-str_extract(paste(dataPSMMat$ScanNumber),"[0-9]+")
				dataPSMMat$Query<-paste(dataPSMMat$Query_tmp,dataPSMMat$ScanNumber,sep="_")
				dataPSMMat$Query_tmp<-NULL
				dataPSMMat$PSM<-paste(dataPSMMat$Query, dataPSMMat$PeptideSeq, sep="_")

				dataPSMMat$PSM<-toupper(dataPSMMat$PSM)
				dataPSMMat$PeptideSeq<-toupper(dataPSMMat$PeptideSeq)

				# dataPSMMat$tmp<-lapply(strsplit(paste(dataPSMMat$datfile),"_summary"),"[",1)
				# dataPSMMat$PeptideSeq<-toupper(dataPSMMat$PeptideSeq)
				# dataPSMMat$Query<-paste(dataPSMMat$tmp,dataPSMMat$ScanString,sep="_")
				# dataPSMMat$PSM<-paste(dataPSMMat$Query,dataPSMMat$PeptideSeq,sep="_")
				# dataPSMMat$tmp<-NULL
