...
datasetTandemTargetFolder=$( echo $DATASET | sed -e  "s/MGFFiles/Omssa_Files/g")
...
TandemTargetFiles=$(find "${datasetTandemTargetFolder}" -iname "*.txt"  -not -name "*_peptides.tsv" -not -name "*_peptides_out.tsv" -not -name "*_corrected.tsv" -not -name "*_peptides_log.log")
echo "OmssaTargetFiles --> "$TandemTargetFiles
...
TandemDecoyFiles=$(find "${datasetTandemDecoyFolder}" -iname "*.txt"  -not -name "*_peptides.tsv" -not -name "*_peptides_out.tsv" -not -name "*_corrected.tsv" -not -name "*_peptides_log.log")
...
if [ $TOD -ne 3 ]; then
	...
	Rscript /home/margaret/data/pepe/scripts/ANALISIS_PROTEOMICA/OmssaPep2Prot_margaret.R $CURRENTFOLDER $datasetname $FILENAME $DBTARGETFILE $DECOYID $TOD
else
