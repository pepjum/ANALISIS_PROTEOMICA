...
tandemFolder <- paste(currentFolder, "Tandem_Files","", sep = "")
...
if (todo == 0 || todo == 1) {
	targetTandemFiles <- dir(path = datasetTandemTargetFolder, pattern = ".tsv")
  ...
  for(i in 1:length(targetTandemFiles)) {
        #cat(targetTandemFiles[i],"\n")
		tandemResult <- read.table(targetTandemFiles[i], header = TRUE, sep = "\t", fill=T)
		write.table(tandemResult$PeptideSeq, file = targetOutTandemFiles[i], quote = FALSE, row.names = FALSE, col.names = FALSE, sep="\t")
			...
			tandemResultMatched <- merge(tandemResult, peptidesMatched, by.x = "PeptideSeq", by.y = "peptide", all.x = TRUE)
...
if (todo == 0 || todo == 2) {
	decoyTandemFiles <- dir(path = datasetTandemDecoyFolder, pattern = ".tsv")
  ...
  for(i in 1:length(decoyTandemFiles)) {
		tandemResult <- read.table(decoyTandemFiles[i], header = TRUE, sep = "\t", fill=T)
