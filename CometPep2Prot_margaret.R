...
if (todo == 0 || todo == 1) {
	...
	for(i in 1:length(targetCometFiles)) {
        ...
        if (is.null(targetCometFiles[i])){
            cat("fichero falla es: ",targetCometFiles[i],"\n")
        }
		if (dim(cosmicResult)[2] == 19) {
			...
			y <- paste('java -XX:ParallelGCThreads=4 -jar /opt/PeptideMatchCMD/PeptideMatchCMD_1.0.jar -a query -i ', currentFolder, '/PeptideMatch_', dbName, '_index -Q ',targetOutCometFiles[i],' -l -e -o ', targetPMOutCometFiles[i], ' > ', targetLogCometFiles[i], sep = "")
			system(y)
			y <- NULL
...
if (todo == 0 || todo == 2) {
	...
	for(i in 1:length(decoyCometFiles)) {
        ...
        if (is.null(targetCometFiles[i])){
            cat("fichero falla es: ",decoyCometFiles[i],"\n")
        }
		...
		if (dim(cosmicResult)[2] == 19) {
			...
			y <- paste('java -XX:ParallelGCThreads=4 -jar /opt/PeptideMatchCMD/PeptideMatchCMD_1.0.jar -a query -i ', currentFolder, '/PeptideMatch_', dbName, '_', decoy_id,'_index -Q ',decoyOutCometFiles[i],' -l -e -o ',decoyPMOutCometFiles[i], ' > ', decoyLogCometFiles[i], sep = "")
			system(y)
			y <- NULL
