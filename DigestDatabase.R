...
cat("initial processing output proteogest","\n")
library(stringr)
...
db_protsAll<-lapply(strsplit(paste(db_protsAll),": "),"[",2)
#db_protsAll<-lapply(strsplit(paste(db_protsAll),"\\|"),"[",2)
#db_protsAll[20547]<-"KKA1_ECOLX"                                   ### PARA OTRAS VERSIONES DE NEXTPROT REVISAR ESTO
db_protsAll<-t(data.frame(db_protsAll))
...
db_NpeptidesAll<-as.numeric(str_extract(db_npeptidesAll,"[0-9]+"))
